...
def process_page(
    ...
    original_columns = layout.columns
    footer_top = layout.footer_top

    image = Image.open(image_path)
    scale_x = image.width / page_width
    scale_y = image.height / page_height
    page_scan_lines = list(scan_lines) if scan_lines is not None else lines
    anchors_by_column: dict[int, list[Anchor]] = {column.index: [] for column in original_columns}
    sections_by_column: dict[int, list[Section]] = {column.index: [] for column in original_columns}
    for anchor in anchors:
        column = assign_column(anchor.x_min, original_columns)
        if column is not None:
            anchors_by_column[column.index].append(anchor)
    for section in sections:
        column = assign_column(section.x_min, original_columns)
        if column is not None:
    ...
    columns = [effective_columns_by_index.get(column.index, column) for column in original_columns]
    columns_by_output_index = {column.index + 1: column for column in columns}

    outputs: list[dict] = []
    debug_rects: list[tuple[str, float, float, float, float]] = []
    records_by_column: dict[int, list[dict]] = {}
    anchors_sorted_by_column: dict[int, list[Anchor]] = {}
    question_blank_tops_by_column: dict[int, list[float]] = {}
    question_tops_by_column: dict[int, list[float]] = {}
    first_record_top_floor_by_column: dict[int, float] = {}
    column_blocks_by_index: dict[int, list[dict]] = {
        column.index: build_column_blocks(
            page_no,
            column,
            image,
            lines,
    ...
    for column in columns:
        column_anchors = sorted(anchors_by_column[column.index], key=lambda anchor: anchor.y_min)
        anchors_sorted_by_column[column.index] = column_anchors
        if not column_anchors:
            continue
        ...
        question_blank_tops_by_column[column.index] = question_blank_tops
        question_tops_by_column[column.index] = question_tops
        for idx, anchor in enumerate(column_anchors):
            ...
            if next_anchor is not None:
                ...
                column_lines = [
                    line
                    for line in lines
                    if line.x_max >= column.x_min
                    and line.x_min <= column.x_max
                    and line.y_min >= top
                    and line.y_min < column_bottom
                ]
                max_line_y = max((line.y_max for line in column_lines), default=top + 40.0)
                bottom = clamp(max_line_y + 24.0, top + 20.0, column_bottom)

            crop_lines = lines_in_rect(lines, column.x_min, column.x_max, top, max(top, bottom - 0.01))
            section_title = ""
            for section in sorted(sections_by_column[column.index], key=lambda item: item.y_min):
                if section.y_min <= anchor.y_min:
                    section_title = section.title

            left_px = int(column.x_min * scale_x)
            top_px = int(top * scale_y)
            right_px = int(column.x_max * scale_x)
            ...
            debug_rects.append((f"Q{anchor.question_no}", column.x_min, top, column.x_max, bottom))
        records_by_column[column.index] = column_records
    ...
    for column_idx in range(1, len(columns)):
        ...
        question_tops = question_tops_by_column.get(column.index, [])
        if not question_tops:
            continue
        continuation_bottom = max(column_top, question_tops[0])

        previous_column = columns[column_idx - 1]
        previous_records = records_by_column.get(previous_column.index, [])
        ...
        column_blocks_by_index[column.index] = remaining_blocks
        previous_record["_blocks"].extend(moved_blocks)
        ...
        pre_anchor_lines = lines_in_rect(lines, column.x_min, column.x_max, column_top, continuation_bottom)
        if pre_anchor_lines:
            previous_record["ocr_text"] = (
                previous_record["ocr_text"].rstrip() + "\n" + normalize_text(pre_anchor_lines).lstrip()
            ).strip()
        for block in moved_blocks:
            debug_rects.append(
                (
                    f"Q{previous_record['question_no']}-cont",
                    block["bbox"]["x_min"],
                    block["bbox"]["y_min"],
                    block["bbox"]["x_max"],
                    block["bbox"]["y_max"],
    ...
    for column in columns:
        column_records = records_by_column.get(column.index, [])
        if not column_records:
            continue

        column_anchors = anchors_sorted_by_column.get(column.index, [])
        question_tops = question_tops_by_column.get(column.index, [])
        boundaries = question_blank_tops_by_column.get(column.index, question_tops)[1:]
        available_blocks = split_blocks_at_boundaries(column_blocks_by_index.get(column.index, []), boundaries)
        for idx, record in enumerate(column_records):
            anchor = column_anchors[idx]
            next_anchor = column_anchors[idx + 1] if idx + 1 < len(column_anchors) else None
            _, column_bottom = column_vertical_bounds[column.index]

            top = question_tops[idx]
            if idx == 0 and column.index in first_record_top_floor_by_column:
                top = max(top, first_record_top_floor_by_column[column.index])
            next_blank_tops = question_blank_tops_by_column.get(column.index, question_tops)
            bottom = column_bottom if next_anchor is None else max(top + 0.1, next_blank_tops[idx + 1] - 0.6)
            assigned = [
                block
                for block in available_blocks
                if top <= (block["bbox"]["y_min"] + block["bbox"]["y_max"]) / 2 < bottom
            ]
            record["_blocks"].extend(assigned)

    for record in outputs:
        part_index = 1
        final_images: list[dict] = []
        unique_blocks: list[dict] = []
        seen_keys: set[tuple[int, int, float, float, float, float]] = set()
        for block in sorted(
            record.pop("_blocks"),
            key=lambda item: (item["page"], item["column"], item["bbox"]["y_min"], item["bbox"]["x_min"]),
        ):
            if block["bbox"]["y_max"] <= block["bbox"]["y_min"] or block["bbox"]["x_max"] <= block["bbox"]["x_min"]:
                continue
            key = (
                block["page"],
                block["column"],
        ...
        final_block_stream = normalize_block_boundaries(unique_blocks)
        if refine_with_band_clusters:
            refined_stream = refine_blocks_with_band_clusters(
                final_block_stream,
                image,
                columns_by_output_index,
                page_scan_lines,
                scale_x,
                scale_y,
            )
            if refined_stream:
                final_block_stream = normalize_block_boundaries(refined_stream)
                final_block_stream = absorb_small_internal_gaps(final_block_stream, page_scan_lines)
                final_block_stream = normalize_block_boundaries(final_block_stream)
        for block_idx, block in enumerate(final_block_stream):
            output_name = build_part_name(page_no, record["question_no"], part_index)
            output_path = crops_dir / output_name
            left_px, top_px, right_px, bottom_px = bbox_to_px(block["bbox"], scale_x, scale_y)
            prev_block = final_block_stream[block_idx - 1] if block_idx > 0 else None
            next_block = final_block_stream[block_idx + 1] if block_idx + 1 < len(final_block_stream) else None
            same_prev_stream = (
                prev_block is not None
                and prev_block["page"] == block["page"]
                and prev_block["column"] == block["column"]
            )
            same_next_stream = (
                next_block is not None
                and next_block["page"] == block["page"]
                and next_block["column"] == block["column"]
            )
            if not same_prev_stream:
                top_px = max(0, top_px - 2)
            if not same_next_stream:
                bottom_px = min(image.height, bottom_px + 2)
            if bottom_px <= top_px or right_px <= left_px:
                continue
            image.crop((left_px, top_px, right_px, bottom_px)).save(output_path)
            final_images.append(
                {
...
if __name__ == "__main__":
    main()
_PAGE_DARK_CACHE: dict[int, np.ndarray] = {}
